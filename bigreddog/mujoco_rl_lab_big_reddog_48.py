...
if __name__ == "__main__":
    ...
    # Get foot geom IDs by name for direct foot position tracking
    foot_geom_names = ["FL_foot", "FR_foot", "RL_foot", "RR_foot"]
    foot_geom_ids = [mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_GEOM, name) for name in foot_geom_names]
    
    # Verify all foot geoms were found
    if -1 in foot_geom_ids:
        print("Warning: Some foot geoms not found.")
        foot_geom_ids = [4, 6, 8, 10]
    else:
        print(f"Found foot geom IDs: {foot_geom_ids} for {foot_geom_names}")
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running() and time.time() - start < simulation_duration:
            ...
            if counter % control_decimation == 0 and counter > 0:
                ...
                # create observation - get foot heights directly from foot geoms
                foot_heights = [d.geom_xpos[geom_id][2] for geom_id in foot_geom_ids]
                ...
                obs = [
                    lin_vel_B * lin_vel_scale,
                    ang_vel_I * ang_vel_scale,
                    gravity_b * gravity_scale,
                    cmd_vel * cmd_scale,
                ...
                joint_vel_list.append((qpos - default_angles) * dof_pos_scale)
